...
def collect_rollouts(model, envs, states, num_steps, device):
    ...
    for state, env in zip(states, envs):
        ...
        for i in range(num_steps):
            ...
            rollout.append((state, action, reward, new_state))
            state = new_state
        rollouts.append(rollout)
    return rollouts, [rollout[-1][3] for rollout in rollouts]
...
def update_model(model, gamma, optim, rollouts, device, iteration, writer):
    ...
    for rollout in rollouts:
        for state, action, reward, new_state in rollout:
            critic_loss += (reward + gamma * model.critic(new_state) - model.critic(state))**2
            actor_loss += -torch.log(model.actor_to_distribution(model.actor(state)).probs[0, action])
    actor_loss /= len(rollouts)
    critic_loss /= len(rollouts)
...
def train_model_ac(model, envs, gamma, device, logdir, val_fn):
    ...
    critic_optim = torch.optim.Adam(model.critic.parameters(), lr=0.001)
