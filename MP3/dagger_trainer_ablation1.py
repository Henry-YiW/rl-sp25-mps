...
def dagger_trainer(env,learner,query_expert,device):
    ...
    max_steps = 200
    epochs=20
    max_dataset_size = 750

    learner.to(device)

    obs_buffer = deque(maxlen=max_dataset_size)
    actions_buffer = deque(maxlen=max_dataset_size)
    ...
    for iteration in range(iterations):
        ...
        while new_obs_list_length < max_steps * num_episodes_per_iter:
            ...
            while not done and steps < max_steps:
                ...
                obs_buffer.append(obs.clone().detach().cpu())
                ...
                if random.random() >= beta:
                    with torch.no_grad():
                        ...
                        action = learner.act(obs.unsqueeze(0)).item()
        ...
        print(f"Episode {episode} of {num_episodes_per_iter}. New obs list length: {new_obs_list_length}")
        learner.train()
        ...
        obs_tensor = torch.stack(list(obs_buffer), dim=0)
        actions_tensor = torch.tensor(list(actions_buffer), dtype=torch.int64)
